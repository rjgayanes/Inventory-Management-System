...
// Build base SQL query (same as in inventories_sep.php)
...
$filename = 'SEP_Inventory_' . date('Y-m-d_H-i-s') . '.csv';
...
// Initialize totals (matching PDF)
$total_balance_qty = 0;
...
// Write data rows (matching PDF data structure)
if ($inventory_result->num_rows > 0) {
    ...
    while ($row = $inventory_result->fetch_assoc()) {
        ...
        // Format article with year (matching PDF)
    ...
    // Add total row (matching PDF)
    fputcsv($output, [
        'TOTAL', '', '', '', '',
        '', // Balance Qnty (empty as in PDF)
        number_format($total_balance_value, 2),
        '', // On Hand Qnty (empty as in PDF)
        number_format($total_onhand_value, 2),
        '', // Short Qnty (empty as in PDF)
        '', // Short Value (empty as in PDF)
        '', // Over Qnty (empty as in PDF)
        number_format($total_over_value, 2),
        ''
    ]);
} else {
    fputcsv($output, ['No Semi-Expendable Properties found']);
}

// Add footer information (matching PDF)
fputcsv($output, []); // Empty row
...
log_activity($conn, $_SESSION['user_ID'], "Exported SEP inventory to Excel");
